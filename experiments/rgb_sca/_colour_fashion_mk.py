...
import sys
NK = int(sys.argv[1]) if len(sys.argv)>1 else 1
kern = F.random_kernels(NK, C=3, K=3, seed=1)
# Valid geometry matching the residual pool, as the anchor does.
pw = np.stack([F.power_features(im, kern, "summed").ravel() for im in rgb])
pw = pw.reshape(len(rgb), NK, -1)[:, :, :ch["n_windows_pool"]].reshape(len(rgb), -1)
# Multiple kernels give multiple INDEPENDENT projections of each (R,G,B) triple.
# With one kernel the summed dataflow yields one scalar per window: one equation, three
# unknowns, so the two chrominance dimensions sit in the null space and no amount of
# averaging recovers them. This is rank deficiency, not noise.
nw = ch["n_windows_pool"]
parts = [NZ.simulate_features_sampled(rgb, kern[k:k+1], "summed", ch, n_avg=1, seed=k,
                                      power=pw.reshape(len(rgb), NK, -1)[:, k, :])
         for k in range(NK)]
feats = np.concatenate(parts, axis=1)
print(f"kernels={NK} -> features {feats.shape}", flush=True)
...
saved = G.save_reconstructions(recs, "experiments/rgb_sca/results/colour_fashion_mk/images",
...
json.dump(out, open("experiments/rgb_sca/results/colour_fashion_mk/results.json", "w"),
          indent=1, default=float)

print(f"\nCOLOUR-FASHION multi-kernel (anchor grayscale reference: MSSIM 0.668)")
