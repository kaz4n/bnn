...
// Leakage-first 3x3 binary convolution engine for CW305/CW-Lite research.
//
...
module mnist_leakage_core #(
    ...
    localparam integer LEAK_BITS = 9 * LEAK_LANES;
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            if (start && !busy) begin
                ...
                if (!dwell_count[0])
                    leak_bank <= {LEAK_LANES{match_bits}};
                else
                    leak_bank <= {LEAK_BITS{1'b0}};
